...
Create the output directories and install skill dependencies (skips if already installed):

```bash
mkdir -p output output/images
```

Then check and install dependencies in this skill directory:

```bash
if [ ! -d "SKILL_DIR/node_modules" ]; then
  cd SKILL_DIR && PUPPETEER_SKIP_DOWNLOAD=true npm install && cd -
fi
```

Replace `SKILL_DIR` with the actual absolute path of this skill directory (the folder containing this `SKILL.md`).
...
**Script path**: relative to this skill directory. Replace `SKILL_SCRIPTS` below with the actual path (e.g., `$OPENCLAW_HOME/.openclaw/skills/pptx/scripts`).
...
```bash
PUPPETEER_EXECUTABLE_PATH="$(which google-chrome)" node SKILL_SCRIPTS/html-to-pptx-dom.js output/filled
```
